...
class Tags(commands.Cog):
    ...
    async def tagcreate(self, ctx, *, tag_to_create: str):
        ...
        if len(split_request) > 1:
            ...
            if tag_name not in self.firebase_ref.child('tags').get():
                ...
                await ctx.send(embed=create_embed(title='That tag already exists. Please choose a different tag name.', colour=Colour.red()))
            return
        await ctx.send(embed=create_embed(description="Couldn't create tag. Please follow this format for creating a tag: **!tagcreate** *NameOfTag* *Content of the tag*.", colour=Colour.red()))
    ...
    async def tagupdate(self, ctx, *, tag_to_update: str):
        ...
        if len(split_update) > 1:
            ...
            return
        await ctx.send(embed=create_embed(description="Couldn't update tag. Please follow this format for updating a tag: **!tagupdate** *NameOfTag* *Updated content of the tag*.", colour=Colour.red()))
    ...
    async def tagdelete(self, ctx, tag_name: str):
        ...
        if self.firebase_ref.child('tags/{0}'.format(tag_name)).get():
            ...
            await ctx.send(embed=create_embed(title="That tag doesn't exist.", colour=Colour.red()))
    ...
    async def tag(self, ctx, tag_name: str):
        ...
        if tag_result:
            ...
            await ctx.send(embed=create_embed(description="That tag doesn't exist. Use **!tagcreate** *tag_name* *Content of the tag* to create a tag.", colour=Colour.red()))